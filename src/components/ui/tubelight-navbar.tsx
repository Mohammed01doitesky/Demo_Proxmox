...
export function NavBar({ items, className }: NavBarProps) {
  ...
  // Calculate active tab based on current pathname
  const getActiveTab = () => {
    // Don't show any active tab on 404 or not-found pages
    if (pathname === '/not-found' || pathname.includes('404')) {
      return null
    }
          ...
          return (
            ...
            >
              ...
              <span className="md:hidden" suppressHydrationWarning>
                <Icon size={18} strokeWidth={2.5} suppressHydrationWarning />
              </span>
              {isActive && activeTab && (
                <motion.div
                  layoutId="lamp"
